...
void ModbusinoSlave::setup (long baud) {
	...
	/*
	  Data bytes are 8 bits long, but there are also stopbits.
	*/
	long bytes_per_second (
	    baud / 10); // If baud % 8 != 0 we will wait a bit longer.
	if (1000000 / bytes_per_second <=
	    3) { // Here and bellow 1000000 if for microseconds in one second.
		g_byte_time = 3;
	}
	else {
		g_byte_time = (1000000 / bytes_per_second + 1000000 % bytes_per_second);
	}

	_byte_time = g_byte_time;
}
...
static int
    build_response_basis (uint8_t slave, uint8_t function, uint8_t *rsp) {
	rsp[ 0 ] = slave;
	rsp[ 1 ] = function;
...
static uint8_t response_exception (
    uint8_t slave, uint8_t function, uint8_t exception_code, uint8_t *rsp) {
	uint8_t rsp_length;
...
int ModbusinoSlave::loop (uint16_t *tab_reg, uint16_t nb_reg) {
	...
	/*
	  There are no other threads, so we can safely assign global variable value
	  of byte time of current object.
	*/
	g_byte_time = _byte_time;
