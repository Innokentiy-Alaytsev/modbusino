...
void ModbusinoSlave::setup(long baud) {
    ...
    */
    long bytes_per_second (baud / 10); // If baud % 8 != 0 we will wait a bit longer.
    if (1000000 / bytes_per_second <= 3) { // Here and bellow 1000000 if for microseconds in one second.
        g_byte_time = 3;
    }
    else {
...
int ModbusinoSlave::loop(uint16_t* tab_reg, uint16_t nb_reg)
{
    ...
    /*
      There are no other threads, so we can safely assign global variable value of byte time of
      current object.
    */
